function myFunction() {
  ...
  if (spread.getActiveSheet().getRange(statusR, 1).getValue() == "RESET") {
    ...
    sp.setProperties({
      "timestamp1": 1620416920,
      "count1": 0
    })
    timestamp = 1620416920
  } else {
    if (sp.getProperty("timestamp1") != 0 && sp.getProperty("timestamp1")) {
      boilerFrom = sp.getProperty("timestamp1") * 1
    } else if (!(sp.getProperty("timestamp1"))) {
      timestamp = 1620416920
    }
    if (sp.getProperty("count1")) {
      count = parseInt(sp.getProperty("count1"))
    }
  }
  ...
  if (Object.keys(emails).length >= 1) {
    for (const x in emails) {
      ...
      MailApp.sendEmail(`3zeet3@gmail.com`, `yvboost ${x} threshold`, `${stringified}`);
    }
  }
  ...
  if (spread.getActiveSheet().getCharts()[0] != null) {
    ...
    chartBuilder.addRange(spread.getActiveSheet().getRange("A1:D8"))
      ...
      .setOption('title', 'CRV to YVBOOST')
